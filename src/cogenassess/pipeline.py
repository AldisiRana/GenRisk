...
def get_gene_info(*, vcf):
    ...
    sub_folder = vcf.split('.')[0]
    if not os.path.exists(sub_folder):
        os.mkdir(sub_folder)
    os.chdir(sub_folder)
    gene_file = sub_folder + '.genes'
    with open(gene_file, 'w') as f:
        for gene in genes:
            f.write("%s\n" % gene)
    [df[df['gene'] == gene][['ID', 'ALT', 'score', 'gene']].to_csv(str(gene) + '.w', index=False, sep='\t') for gene
     in
     list(set(df['gene']))]
    [df[df['gene'] == gene][['ID']].to_csv(str(gene) + '.v', index=False, sep='\t') for gene in
     list(set(df['gene']))]
    return sub_folder
...
def plink_process(*, genes_folder, plink, bed, bim, fam):
    ...
    for gene in tqdm(genes, desc='calculating genes scores'):
        p = subprocess.call(
            plink + " --bed " + bed + " --bim " + bim + " --fam " + fam + " --extract " + gene + ".v --score "
            + gene + ".w 1 2 3  sum --out " + gene)
